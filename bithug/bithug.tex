...
In the Ruby world, there are some KV-Stores worth looking into, some of which 
are fast and simple to use, others which have a more advanced API but provide 
additional features that go beyond a "simple" hashing KV-Store. One of the most 
prominent solutions might be MongoDB. It is a document-oriented database, 
meaning its data is composed of simple key-value constructs that together form a 
larger chunk of data. It is quite simple to use and provides much of the querying 
capabilities of the traditional SQL database, though it's queries are not as 
pretty as Redis'. Though it is reasonably fast, MongoDB cannot compete with the 
blazing speed of simpler databases.
